...
int   b  = 1   ,
      ...
      x , y ,  ax , ay , Score , Eats,Good_Eats, Good_Eats_Per  , Hyper_Count , body     , 
      xbody[150]  , ybody[150] , Active_Temp   , Hyper_Mode     , Switch_Hub  , Gameover , 
      Tripping    , Immortal   , epilepsy      , Hide_Body      , Zig_Zag     , Zag_Zig  ,  Pause ,         
...
void Back(int Forgetten = 0)
{
     ...
     while(Loop_1 != Power)
     {
          if( Forgetten ) while( !kbhit() ) Move_Stars() ;
          Loop_1 = getch() ;
      }
     cls() ; 
}
...
void Credits()
{
     ...
     cout << "\n\n" ; 
     spaceing(Xspaceing_Credits - strlen(Class)/2 - 1) ; framepause(15000)  ; 
     Loop_1 = -1  ; while(++Loop_1 < strlen(Class)) { cout << Class[Loop_1] ; framepause(2500) ; } cout << "\n"   ;
     spaceing(Xspaceing_Credits - strlen(Batch)/2) ; framepause(15000)  ; 
     Loop_1 = -1  ; while(++Loop_1 < strlen(Batch)) { cout << Batch[Loop_1] ; framepause(2500) ; } cout << "\n"   ; 
     spaceing(Xspaceing_Credits - strlen(Quote)/2 )  ; framepause(15000)      ;
     Loop_1 = -1  ; while(++Loop_1 < strlen(Quote)) { cout << Quote[Loop_1] ; framepause(3500) ; } cout << "\n\n" ;
...
{    
     ...
     Space = Start_Space ; 
	 dir = ' ' ;
	 for(int Loop_1 = 0 ; Loop_1 < body ; Loop_1 ++){xbody[Loop_1] = -1 ;ybody[Loop_1] = -1 ;}
	 body  = 1   ;
