...
def dot(x,y) :
    global PLANETS
    PLANETS.append(Star(1250000, Vec(x,y), Vec(0,-195), gs, "circle"))
def main():
    gs = GravSys()
    make_planet_shape("Planet" , "white" , "white")
    def dot(x,y) :
        global PLANETS , p
        PLANETS.append(Star(1250, Vec(x,y), Vec(0,y*2), gs, "circle"))
        PLANETS[p].color("white")
        PLANETS[p].pu()
        p += 1
    Sun = Star(1250000, Vec(0,0), Vec(0,0), gs, "circle")
    Sun.color("gold")
    Sun.shapesize(1.6)
    Sun.pd()
